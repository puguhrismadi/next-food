...
function Dashboard() {
   ...
   // console.log(token);
    ...
    const fetchData = async () => {
        ...
        await axios.get(`${process.env.NEXT_PUBLIC_API_BACKEND}/api/users/me`)
        .then((response) => {
    ...
    const logoutHanlder = async () => {
        ...
        //set axios header dengan type Authorization + Bearer token
        axios.defaults.headers.common['Authorization'] = `Bearer ${token}`
        //fetch Rest API -- await axios.post(`${process.env.NEXT_PUBLIC_API_BACKEND}/api/logout`)
        await axios.post(`/logout`)
        .then(() => {
            //remove token from cookies token
